...
public class ApiParameterInspectionTest extends EndpointTestBase {
  ...
  /**
   * Tests that no ApiParameterInspection errors are generated for constructors with parameters that
   * are of API parameter type.
   */
  ...
  /**
   * Tests that {@link ApiParameterInspection.MyQuickFix} applies the {@link
   * GctConstants.APP_ENGINE_ANNOTATION_NAMED} annotation to a {@link PsiParameter} when the {@link
   * PsiParameter} does not already have an @Named annotation.
   */
  ...
  /**
   * Tests that {@link ApiParameterInspection.MyQuickFix} does not add any annotation to a {@link
   * PsiParameter} that already has an @Named annotation.
   */
  public void testQuickFix_parameterWithNamedAnnotation() {
    Project myProject = myFixture.getProject();
    String parameterText =
        "@" + GctConstants.APP_ENGINE_ANNOTATION_NAMED + "(\"foo\")int someParam";
  ...
  private void runQuickFixTest(String parameterText, String expectedString) {
    ...
    PsiParameter parameter =
        JavaPsiFacade.getInstance(myProject)
            .getElementFactory()
            .createParameterFromText(parameterText, null);
  ...
  private void doTest() {
    ...
    myFixture.testInspection(
        "inspections/apiParameterInspection/" + testName,
