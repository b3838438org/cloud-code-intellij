...
public class AppEngineFlexibleDeploymentEditor
  ...
  public AppEngineFlexibleDeploymentEditor(Project project, AppEngineDeployable deploymentSource) {
            ...
            new DocumentAdapter() {
              ...
              protected void textChanged(DocumentEvent event) {
                if (deploymentSource instanceof UserSpecifiedPathDeploymentSource) {
                  ((UserSpecifiedPathDeploymentSource) deploymentSource)
                      .setFilePath(archiveSelector.getText());
                }
              }
            });

    appYamlOverrideCheckBox.addActionListener(
        event -> {
    ...
    gcpProjectSelector.addModelListener(
    ...
    promoteVersionCheckBox.addItemListener(
        event -> {
    ...
    modulesWithFlexFacetComboBox.setModel(
    ...
    modulesWithFlexFacetComboBox.setRenderer(
    ...
    moduleSettingsButton.addActionListener(
        event -> {
  ...
  private void refreshApplicationInfoPanel() {
    if (gcpProjectSelector.getProject() != null && gcpProjectSelector.getSelectedUser() != null) {
      appInfoPanel.refresh(
          gcpProjectSelector.getProject().getProjectId(),
          gcpProjectSelector.getSelectedUser().getCredential());
    } else {
      appInfoPanel.setMessage(
  ...
  protected void resetEditorFrom(@NotNull AppEngineDeploymentConfiguration configuration) {
    ...
    appYamlTextField.setVisible(
        configuration.isOverrideAppYaml() || modulesWithFlexFacetComboBox.getItemCount() == 0);
    archiveSelector.setText(configuration.getUserSpecifiedArtifactPath());
    appYamlOverrideCheckBox.setSelected(
        configuration.isOverrideAppYaml() || modulesWithFlexFacetComboBox.getItemCount() == 0);
    dockerfileOverrideCheckBox.setSelected(
        configuration.isOverrideDockerfile() || modulesWithFlexFacetComboBox.getItemCount() == 0);
  ...
  private void validateConfiguration() throws ConfigurationException {
    ...
    if (StringUtils.isBlank(getAppYamlPath())) {
      throw new ConfigurationException(GctBundle.message("appengine.flex.config.browse.app.yaml"));
    }
  ...
  private void setDeploymentProjectAndVersion() {
    if (deploymentSource instanceof AppEngineDeployable) {
      ...
      ((AppEngineDeployable) deploymentSource)
          .setVersion(Strings.isNullOrEmpty(version.getText()) ? "auto" : version.getText());
    }
  }
