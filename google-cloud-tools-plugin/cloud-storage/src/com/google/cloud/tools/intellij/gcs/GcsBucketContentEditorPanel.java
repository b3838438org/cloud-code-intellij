...
final class GcsBucketContentEditorPanel {
  ...
  void initTableModel() {
    if (!Services.getLoginService().isLoggedIn()) {
      showMessage(CloudStorageMessageBundle.message("gcs.content.explorer.not.logged.in.text"));
      return;
    }
    ...
    Consumer<List<Blob>> afterLoad =
        blobs -> {
          if (blobs.isEmpty()) {
            showMessage(
                CloudStorageMessageBundle.message("gcs.content.explorer.empty.bucket.text"));
          } else {
            showBlobTable();
