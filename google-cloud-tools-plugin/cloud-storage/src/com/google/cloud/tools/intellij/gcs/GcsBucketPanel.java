...
final class GcsBucketPanel {
  ...
  void refresh() {
    ...
    if (!Services.getLoginService().isLoggedIn()) {
      notificationLabel.setText(
          CloudStorageMessageBundle.message("gcs.panel.bucket.listing.not.logged.in"));
    } else if (projectSelector.getSelectedProject() == null) {
      notificationLabel.setText(
          CloudStorageMessageBundle.message("gcs.panel.bucket.listing.no.project.selected"));
    } else {
      ...
      if (user.isPresent()) {
        ...
        notificationLabel.setText(
            CloudStorageMessageBundle.message("gcs.panel.bucket.listing.error.loading.buckets"));
  ...
  private void loadAndDisplayBuckets(String projectId, CredentialedUser credentialedUser) {
    ...
    notificationLabel.setText(
        CloudStorageMessageBundle.message("gcs.panel.bucket.listing.loading.text"));
                ...
                () -> {
                  ...
                  try {
                    ...
                    if (Iterators.size(buckets.iterator()) == 0) {
                      notificationLabel.setText(
                          CloudStorageMessageBundle.message(
